...
while 1:
url_pass_code = "http://kyfw.12306.cn/otn/passcodeNew/getPassCodeNew?module=%s&rand=%s" % (
    module, rand_method)
...
if module == 'passenger':
self.proxy_ext_header[
    "Referer"] = "http://kyfw.12306.cn/otn/confirmPassenger/initDc#nogo"
...
g_conn.request('POST', url_login, body=post_data,
               headers=self.proxy_ext_header)
res = g_conn.getresponse()
...
url_query = "http://kyfw.12306.cn/otn/leftTicket/query?" + \
    urllib.urlencode(g_query_data)
...
g_conn.request('POST', url_queue_count, body=post_data,
               headers=self.proxy_ext_header)
res = g_conn.getresponse()
data = res.read()
res_json = json.loads(data)
...
# print
# "cancel_flag=2&bed_level_order_num=000000000000000000000000000000&passengerTicketStr=1%2C0%2C1%2C%E6%9C%B1%E5%AD%94%E6%B4%8B%2C1%2C320721198711180812%2C13430680458%2CN&oldPassengerStr=%E6%9C%B1%E5%AD%94%E6%B4%8B%2C1%2C320721198711180812%2C1_&tour_flag=dc&randCode=ewgw&_json_att=&REPEAT_SUBMIT_TOKEN=ad51ea02d933faf91d3d2eaeb5d85b3e"
g_conn.request('POST', url_check_order, body=post_data,
               headers=self.proxy_ext_header)
res = g_conn.getresponse()
data = res.read()
res_json = json.loads(data)
...
g_conn.request('POST', url_check_info, body=post_data,
               headers=self.proxy_ext_header)
res = g_conn.getresponse()
data = res.read()
res_json = json.loads(data)
...
g_conn.request('POST', url_submit, body=post_data.encode(
    "utf8"), headers=self.proxy_ext_header)
res = g_conn.getresponse()
data = res.read()
res_json = json.loads(data)
if res_json['status'] != True:
...
g_conn.request('POST', url_result, body=post_data,
               headers=self.proxy_ext_header)
res = g_conn.getresponse()
data = res.read()
res_json = json.loads(data)
...
g_conn.request('POST', url_get_passager_info,
               body=post_data, headers=self.proxy_ext_header)
res = g_conn.getresponse()
data = res.read()
res_json = json.loads(data)
