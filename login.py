...
class LoginTic(object):
    ...
    def checkYanZheng(self, solution):
        ...
        yanSol = ['35,35', '105,35', '175,35', '245,35',
        ...
        cont = self.session.post(
            url=checkUrl, data=data, headers=self.headers, verify=False)
    ...
    def loginTo(self):
        ...
        result = self.session.post(
            url=loginUrl, data=data, headers=self.headers, verify=False)
        dic = loads(result.content)
        print result.content
        mes = dic['result_message']
        # 结果的编码方式是Unicode编码，所以对比的时候字符串前面加u,或者mes.encode('utf-8') ==
        # '登录成功'进行判断，否则报错
        if mes == u'登录成功':
            print '恭喜你，登录成功，可以购票!'
        else:
